...
/**
 * Dev-only mock of the Go backend. Implements the AG-UI SSE contract so the
 * widget streams realistically without any AI provider. Mounted as Vite
 * middleware at POST /agent/run.
 */
...
function buildReply(prompt: string): string {
  ...
  return (
    ...
    'transport → core store → renderers → UI. Try asking about the `weather` ' +
    'to see a tool-call rendered.'
  );
}
